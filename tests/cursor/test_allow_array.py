...
def test_array_enabled_false_default_raises_on_select(sqream_cursor, data_type):
    ...
    with pytest.raises(ArraysAreDisabled):
        select(sqream_cursor, TEMP_TABLE)


@pytest.mark.parametrize("data_type, data", zip(ALL_TYPES, SIMPLE_VALUES))
def test_array_enabled_false_default_raises_on_network_insert(sqream_cursor, data_type, data):
